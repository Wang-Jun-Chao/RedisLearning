 ...
 * All Rights Reserved !!!
 */
public class SInter extends Command<String, String> {
    @Test
    @Override
    public void testTemplate() {
        template.opsForSet().add("key1", "a", "b", "c");
        template.opsForSet().add("key2", "c", "d", "e");
        Set<String> diff = template.opsForSet().intersect("key1", "key2");
        System.out.println(diff);
        Set<String> expected = Sets.newHashSet("c");
    ...
    public void testConnection() {
        template.opsForSet().add("key1", "a", "b", "c");
        template.opsForSet().add("key2", "c", "d", "e");
        Set<byte[]> byteDiff = connection.sInter(keySerializer.serialize("key1"),
